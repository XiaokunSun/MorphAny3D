    ...
    def _forward(self, x: torch.Tensor, mod: torch.Tensor, context: torch.Tensor, step_idx: int, block_idx: int, **kwargs):
        ...
        if len(kwargs) > 0:
            if kwargs["ss_mca_flag"]:
                h = feature_interp(self.cross_attn(x=h, context=context, step_idx=step_idx, block_idx=block_idx), self.cross_attn(x=h, context=kwargs["tar_cond"], step_idx=step_idx, block_idx=block_idx), kwargs["alpha"], interp_mode="linear")
            else:
